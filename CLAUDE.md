...
| 图 | 横轴 | 纵轴 | 高度 |
|---|---|---|---|
| 瞄准收益曲线 | 瞄准回合（索引） | 50%好击距离（格） | 340 |
| 瞄准时间线里的图 | 瞄准回合（索引） | 瞄准误差 | 560 |
| 误差与命中档位 | **瞄准误差**（自定义刻度） | 各档位概率（%） | 340 |

前两张的横轴是采样点下标（就是回合数），第三张**不是** —— 它的横轴是瞄准误差
本身，而且采样点刻意非等距（低误差段密、高误差段疏，因为概率的变化几乎都
发生在低段）。所以 `ChartOpts` 有两个为此加的东西：

- `x_labels`：**每个点都要填**的全量刻度文字。悬停提示取的是那个点的准确值，
  所以要全量；轴上画多少由 `x_tick_step` 单独控制。
- `x_label`：横轴名。默认是「瞄准回合」，那张图传「瞄准误差」。

> 踩过：一开始 `x_labels` 只给每 3 个点填了值（想着「反正轴上只画这些」），
> 结果悬停提示读到空字符串，标题变成光秃秃的「瞄准误差」。

### 可拖拽的控件：哨兵值和 clamp 不能放在一起

列表宽度、图表高度这些都是「用户拖过之后才有效」的值，用 **0 当哨兵**
表示「还没拖过，用默认值」。坑在于：**clamp 一旦无条件每帧执行，就会在
第一帧把哨兵夹成下限**。

```cpp
const float H = ( view.height > 0.0f ) ? view.height : opts.height;
if( gact ) { view.height = H + io.MouseDelta.y; }
view.height = std::clamp( view.height, 140.0f, 1400.0f );   // ← 这行每帧跑就完了
```

表现是所有图都变成最矮的 140px（用户截图报上来的）。**clamp 要放进
`if (gact)` 里面**，只在拖动时做。

（左右分栏那条不受影响 —— `g_list_width` 的初值 380 本来就落在
[200, 上限] 里，夹不出问题。）

> 另一处：`ChartOpts` 字段一多，**聚合初始化 `{a, b, c}` 就成了雷** ——
> 我在中间插了个 `x_label`，两个老调用点立刻错位（字符串被当成指针传给
> `marks`），编译才报出来。三个调用点现在都改成具名赋值，加字段不再错位。

### 「误差与命中档位」那张图统计的是**真实分布**，不是纯阈值分档

`tier_index()` 是纯按阈值分桶（好击/普通/擦伤三档确实如此），但游戏对
**爆头和暴击有额外判定**（0.I `creature.cpp:1133-1180`）。所以那张图
（只有它）另外算了一遍：

| 档位 | 游戏里的条件 | 这张图怎么算 |
|---|---|---|
| 爆头 | `g<0.1` 且（命中头部 **或** `fatal_hit`） | **不给这条线**，见下 |
| 暴击 | `g<0.2` 且过 `crit_roll` | `P = (0.4−g)/(1−g)` |
| 好击 | `g<0.5` | 该档 **加上**没过 `crit_roll` 掉回来的那部分 |
| 普通/擦伤/脱靶 | 纯阈值 | 同 `tier_index()` |

**为什么给不出爆头**：命中头部要目标的部位/弱点数据（玩家走 `targeting_graph`
部位图、怪物走 `select_weakpoint` 弱点表）；`fatal_hit` 要「伤害 × 暴击倍率 >
目标血量」，要伤害和血量。**这些 gunlab 都没建模，而且随目标而变**（打狗和
打巨兽完全不同）。

> 顺带查清一件事：玩家作目标时，`goodhit < 0.1` 那个分支里**部位图永远选不中
> 头**。路径是 `[躯干, X]`，X 按 hit_size 加权（手臂13×4/腿13×4/头4，共56），
> 选中头时总权重 40、缩放 2/40、躯干先吃掉 1.8；而 `value'` 最大只有 1.1。
> 所以游戏里的 `Critical!!` 实际主要来自 `fatal_hit` —— 也就是「一枪能打死」。

> 游戏源码里 `crit_roll` 那行的注释写「20% true when goodhit = 0.2」，
> **是错的** —— 按代码算是 25%。已用蒙特卡洛验证过解析式
> （200 万次采样，各点误差 < 0.0005）。以代码为准，别信那句注释。
...
普通档 2 格 325 行动点 / 仔细档 3 格 483 / 精准档 4 格 709
```

> 注意：**瞄准用时**那一行在 2026-09-11 改过一次（仔细档 482 → 483），
> 因为发现阈值该按游戏的 `int` 截断（见下）。其余三项（散布 / 瞄准散布 /
> 后坐）没动 —— 那几个是照着游戏物品面板核对过的。

### 瞄准档位的阈值：两个必须照抄的细节

游戏在 `ranged.cpp:2211 Character::get_aim_types()` 里定三个档位：把
「瞄准精度上限 → MAX_RECOIL」这段按比例切，**10% / 2.5% / 0%**。

1. **阈值是 `int`。** `aim_type::threshold` 是 int，`gun_engagement_moves()`
   也收 int。以前 gunlab 用未截断的 double 比，会让行动点数**偏小** ——
   实测 332 把枪里 **199 把的仔细档各差 1 点**（127.65 截成 127 就得多走
   一步），普通档 5 把。已改成 int。
2. **重复的阈值会被去掉。** 高散布武器（上限接近 MAX_RECOIL）可能只剩一
   两档。`AimResult::has_careful / has_precise` 标记档位存不存在，界面
   （瞄准等级表、瞄准时间线的参考线、瞄准档位实例）都要跳过不存在的那行。
   当前 332 把枪一把都不会触发（最高上限是 `ar12_shotgun` 的 200，而要塌缩
   得 > 2960），属于防御性代码 —— 但加 mod 带进高散布武器就会用上。

> 游戏注释又错一处：`get_aim_types()` 写「at 10%, 5% and 0%」，但代码是
> `/10` 和 `/40`，实际是 **10% 和 2.5%**。以代码为准。
> 这已经是本项目第三次撞见游戏注释与代码不符 —— 前两次是 `crit_roll` 的
> 概率、和 `min_cycle_recoil` 那条「后续打不出」。
...
- **「总散布」比「瞄准误差」多一个常数** —— 那是"固定散布"
  ...
  表里已单列出来。
- **同一个百分比在多行重复出现** —— 数学必然：偏移量与距离成正比、
  判定阈值也随距离等比放大，两者抵消，不同距离的不同档位会卡在同一个
  散布投骰阈值上。（这是老的概率表里的现象，表已删；道理不变。）
