...
inline const char *TIERCURVE_HINT =
    "横轴是瞄准误差本身（不是回合）。距离可调 —— 命中档位不只由误差决定：未命中度 = 横向偏移 ÷ 目标体积，而偏移随距离线性放大，所以同一个误差在 5 格和 40 格完全是两回事。\n"
    "★ 左端不会升到 100%：误差降到 0 也还有「固定散布」垫着（枪身+弹药散布 ÷18、敏捷修正、技能不足惩罚 —— 就是「瞄准参数」里的『总散布』）。技能低时这一项极大：技能 0 的 AKM 是 287.5，而枪自身散布只有 12 —— 这时候卡住你的是技能，不是枪。\n"
    "★ 「暴击」画的是真实比例：未命中度够低只是进入该档，还要过一道 crit_roll（概率 = (0.4−g)/(1−g)），没过的那部分会掉回「好击」。\n"
    "★ 没有「爆头」这条线：游戏里那一档要 goodhit < 0.1 且（命中头部 或 致命一击）。命中头部要目标的部位或弱点数据，致命一击要「伤害 × 暴击倍率 > 目标血量」—— 这两样本工具都没建模，而且它们随目标而变（打狗和打巨兽完全不同），给不出通用数字";
